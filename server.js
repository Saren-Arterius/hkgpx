...
var checkAPIRequest = function (req, res, next) {
  ...
  if (!checkRateLimit('account_action', req.headers['x-real-ip'], ACCOUNT_ACTION_RATE_LIMIT_TIMES, false)) {
  ...
  if (req.params.private_token !== db['accounts'][req.params.id]['private_token']) {
    ...
    checkRateLimit('account_action', req.headers['x-real-ip'], ACCOUNT_ACTION_RATE_LIMIT_TIMES, true);
    return;
  }
  next();
};
...
var topicListJsonFromDoc = function (doc) {
  ...
  $('[id*=Thread_No]').each(function () {
    ...
    var topicID = parseInt($(this).children().eq(1).children().eq(0).attr('href').split('=')[2], 10);
    ...
    var replies = parseInt($(this).children().eq(4).text().trim().replace(',', ''), 10);
    var rating = parseInt($(this).children().eq(5).text().trim().replace(',', ''), 10);
    var message = {
      'Message_ID': topicID,
...
var topicJsonFromDoc = function (doc) {
  ...
  if (ratingContainer.length) {
    ratedGood = parseInt(ratingContainer.children().eq(1).text(), 10);
    ratedBad = parseInt(ratingContainer.children().eq(3).text(), 10);
  }
  var stat = $('#ctl00_ContentPlaceHolder1_view_form').children().eq(2).text().indexOf('鎖') !== -1 ? 'L' : 'A';
  var rep = parseInt($('td.repliers_header:nth-child(2) > div:nth-child(2)').text().split('個')[0], 10);
  var json = {
    'success': true,
    'Message_Title': title,
    'Message_Status': stat,
  ...
  $('.repliers').each(function () {
    ...
    if (replyInfo.length !== 0) {
      var userID = parseInt(replyInfo.attr('userid'), 10);
      var username = replyInfo.attr('username');
      var postID = 1;
      var postIcon = 0;
      var genderIdentity;
      if (replyInfo.children().eq(0).children().eq(0).children().eq(0).attr('name')) {
        postID = parseInt(replyInfo.children().eq(0).children().eq(0).children().eq(0).attr('name'), 10);
      ...
      if (iconSrc) {
        postIcon = parseInt(iconSrc.replace('icons/', '').replace('.gif', ''), 10);
      }
      if (postIcon !== 0) {
...
server.put('/new-account/:id/:private_token', checkParams, function (req, res, next) {
  ...
  if (!checkRateLimit('account_action', req.headers['x-real-ip'], ACCOUNT_ACTION_RATE_LIMIT_TIMES, true)) {
    res.send(429, 'Rate limit exceeded.');
    return;
  }
  if (FRIEND_ONLY_SERVER && FRIEND_USER_IDS.indexOf(parseInt(req.params.id, 10)) === -1) {
...
server.get('/topic-list/:forum/:page/:id/:private_token', checkParams, checkAPIRequest, function (req, res, next) {
  var isFriend = FRIEND_USER_IDS.indexOf(parseInt(req.params.id, 10)) !== -1;
      ...
      (useAPI ? apiLimiter : desktopLimiter).removeTokens(1, function (err, remainingRequests) {
        ...
        request(options, function (error, response, body) {
          ...
          if (cache['data']['messages'].length === limit &&
            cache['data']['Total_Replies'] > (page + 1) * POSTS_PER_PAGE) {
...
server.post('/raw-request/:id/:private_token', checkParams, checkRawRequest, checkAPIRequest, function (req, res, next) {
  if (FRIEND_USER_IDS.indexOf(parseInt(req.params.id, 10)) === -1) {
    if (!checkRateLimit('hkg_access', req.params.private_token, API_ACCESS_RATE_LIMIT_TIMES, true)) {
      res.send(429, 'Rate limit exceeded.');
      return true;
    }
  ...
  (req.body.api ? apiLimiter : desktopLimiter).removeTokens(1, function (err, remainingRequests) {
    ...
    request(options, function (error, response, body) {
      if (error !== null && 'code' in error && error.code === 'ETIMEDOUT') {
        ...
        addStat(req.body.api ? apiSuccesses : forumSuccesses, 0);
        return;
      }
      if (error || response.statusCode !== 200) {
